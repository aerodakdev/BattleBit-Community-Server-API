...
using CommunityServerAPI.BattleBitAPI.Discord;
...
namespace BattleBitAPI.Server
{
    public class GameServer<TPlayer> : System.IDisposable where TPlayer : Player<TPlayer>
    {
        ...
        {
            Console.Title = $"[BBRSF] Players: {AllPlayers.Count()} | RAM: {Process.GetCurrentProcess().PrivateMemorySize64 / (1024 * 1024)} | Map: {Map}";
        }
        ...
        {
            string webHookURL = "https://api.discord.com/webhook/A12345/B12345";
            from.WarnPlayer($"{to.Name} is successfully reported to server admins\nFor {reason.ToString()}\nThanks for your report!");
            Discord.sendWebhookMessage(
                /*  The Message will be sent from our API to Discord Webhook you set  */
                $"{to.Name} is reported by {from.Name} for {reason.ToString()}\nReport Note: {additional}",
                /*  Discord Webhook URL ( you can get the URL from your webhooks page at server settings )  */
                webHookURL,
                /*  Discord Webhook User Name  */
                "Battle Bit Server Reports",
                /*  Avatar Icon ( Looks cool with a icon )  */
                "https://cdn.discordapp.com/attachments/1134216799332880574/1140746122340605972/thumbnail.png"
